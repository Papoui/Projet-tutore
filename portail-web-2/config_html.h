...
<body>
    ...
    <form id="configForm" onsubmit="event.preventDefault(); saveConfig();">
        Texte: <input type="text" id="text" name="text"><br>
        Entier: <input type="number" id="integer" name="integer"><br>
        Flottant: <input type="number" step="0.1" id="float" name="float"><br>
        Booléen: <input type="checkbox" id="boolean" name="boolean"><br>
    ...
    <script>
        ...
        function loadConfig() {
            ...
            fetch('/api/config')
                ...
                .then(data => {
                    ...
                    document.getElementById('boolean').checked = data.boolean;
        ...
        function saveConfig() {
            ...
            params.append('integer', document.getElementById('integer').value);
            params.append('float', document.getElementById('float').value);
            // Pour les checkbox, on envoie '1' si coché, sinon on n'envoie rien (ou '0')
            if(document.getElementById('boolean').checked) params.append('boolean', '1');
